...
function UserMessage({ text }: { text: string }) {
  return (
    <div className="flex justify-end">
      ...
      >
        <p className="text-sm leading-relaxed" style={{ color: "#e5e7eb" }}>{text}</p>
      </div>
    </div>
  );
}
    ...
    <div
      ...
      onMouseLeave={(e) => {
        if (canSelect && !selecting)
          e.currentTarget.style.borderColor = hasError ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.06)";
      }}
    >
      {/* Model header */}
      <div
        ...
        <div className="flex items-center gap-2">
          ...
          {result.validation_success ? (
            <span
              ...
              style={{ background: "rgba(34,197,94,0.1)", color: "#4ade80", fontSize: "10px" }}
            >
              valid
            </span>
          ) : (
            <span
              className="rounded-full px-1.5 py-0.5"
              style={{ background: "rgba(239,68,68,0.1)", color: "#f87171", fontSize: "10px" }}
            >
      ...
      >
        <div className="flex gap-4">
          {result.total_tokens > 0 && (
            <>
              <span className="text-xs" style={{ color: "#4b5563" }}>in: {result.input_tokens}</span>
              <span className="text-xs" style={{ color: "#4b5563" }}>out: {result.output_tokens}</span>
              <span className="text-xs" style={{ color: "#4b5563" }}>total: {result.total_tokens}</span>
            </>
          )}
        </div>
        {canSelect && (
      ...
      <div className="w-full max-w-4xl space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs" style={{ color: "#4b5563" }}>
            {data.model_results.length} model{data.model_results.length !== 1 ? "s" : ""} &middot; click a config to simulate
          </span>
        </div>
  ...
  return (
    ...
    >
      ...
      {selecting && (
        ...
        >
          <div className="flex items-center gap-3 rounded-2xl px-6 py-4" style={{ background: "#1a1a22" }}>
            <Loader2 size={20} className="animate-spin" style={{ color: "#6b7280" }} />
            <span className="text-sm" style={{ color: "#d1d5db" }}>Applying config to CARLA...</span>
          </div>
        </div>
      )}
            ...
            <div className="w-full max-w-2xl space-y-10">
              <div className="space-y-3 text-center">
                ...
                <p className="text-sm" style={{ color: "#6b7280" }}>
                  MetisCity generates CARLA configs using Gemini, Claude, GPT, and DeepSeek in parallel.
                  <br />
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 pb-3">
            ...
            <button
              ...
              style={{
                background: input.trim() && !loading ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.03)",
                color: input.trim() && !loading ? "#e5e7eb" : "#4b5563",
                cursor: input.trim() && !loading ? "pointer" : "not-allowed",
              }}
              onMouseEnter={(e) => {
                if (input.trim() && !loading) e.currentTarget.style.background = "rgba(255,255,255,0.15)";
              }}
              onMouseLeave={(e) => {
                if (input.trim() && !loading) e.currentTarget.style.background = "rgba(255,255,255,0.1)";
              }}
            >
